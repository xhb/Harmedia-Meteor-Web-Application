/*
Figure out a better way to deal with viewers
Look into this: https://github.com/francisbyrne/hangwithme/blob/master/server/game.js
*/
...
Router.route('/c/:channelURL', {
  ...
  waitOn : function() {
    ...
    try {
        ...
        //console.log("HFKDSFJLDSFJDKSLFJDSLKFJKL WORKING SUBBING!");
    }
    catch(e) {
      console.log("Unable to subscribe to getBannedAndSilenceUser!");
    }

    //remove me later and mamke me better
...
Router.route('/c/:channelURL/edit', {
  ...
  /*onBeforeAction: function() {
    var self = this.next;
    Meteor.call('isMyChannel', Meteor.user().username, this.params.channelURL, function(error,result) {
      if (error || !result) {
        ...
        var self = this.next; //going to the page
        console.log("Nice this is your channel!");
      }
    });
  }*/
});
